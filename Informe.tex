...
\end{itemize}
Y la reconstrucción,para el caso de 2D se visualiza en la figura \ref{bancofiltrosR}
\begin{figure}[H]
	\centering
	...
	\label{bancofiltrosR}
\end{figure}
Donde los filtros LR =LoR y HR=HoR hacen referencia en los filtros pasa bajo y pasa alto descriptos en la ecuación \ref{recomp}, y los bloques $2\uparrow$ representan el rellenado de la señal para obtener la dimension correcta en la reconstrucción.
...
\section{Resultados}
A continuación, presentamos los resultados obtenidos para la siguientes imágenes de prueba prensentadas en la figura \ref{fig-imagenes-prueba}.Todas se modificaron con un ruido gaussiano de $\sigma=0.3$.\\
...
\end{itemize}
obteniéndose las imágenes filtradas y reconstruidas presentadas en la figura \ref{fig-imagenes-filtradas}.
...
Para corrobar este resultado presentamos tablas comparativas de las distintas combinaciones de parámetros descritos en el item 4-iii en la sección \ref{asd}. Los estadísticos que se utilizaron para comparar es el PSNR (Peak Signal-to-Noise Ratio) que define la relación entre la máxima energía posible de una señal y el ruido que la afecta, mientras menor es el valor mayor es la cantidad de ruido en la imagen. El otro estadístico utilizado es el SSIM (structural similarity) que mide la similitud entre dos imágenes. El resultado varia entre $0$ y $1$, donde $SSIM=1$ indica que las imágenes son iguales. La ventaja de este estadístico recae en que los resultados coinciden con la percepción del ojo humano, lo que no sucede con el PSNR.
...
\end{tabular}
\label{tabla-levels}
\caption{Proporción máxima de señal a ruido utilizando wavelet Sym8, umbral normalshrink, metodo soft.}
\end{table}
\begin{table}
\centering 
\begin{tabular}{lrrrrr}
...
\caption{SSIM utilizando wavelet Sym8, umbral normalshrink, metodo soft.}
\end{table}
\begin{figure}
        \centering
...
\end{tabular}
\label{tabla-modos}
\caption{Proporción máxima de señal a ruido utilizando wavelet Sym8,dos niveles de descomposición,umbral normalshrink.}
\end{table}
\begin{table}
\centering 
\begin{tabular}{lrrr}
...
\caption{Proporción máxima de señal a ruido utilizando wavelet Sym8,dos niveles de descomposición,metodo soft.}
\end{table}
\begin{table}
\centering 
\begin{tabular}{lrrrrrr}
...
\caption{SSIM utilizando wavelet Sym8,dos niveles de descomposición,metodo soft.}
\end{table}
\begin{figure}
        \centering
        \begin{subfigure}[H]{0.22\textwidth}
...
\subsection{Comparacion con otros metodos}
...
\caption{Proporción máxima de señal a ruido para los distintos metodos de filtrado.}
\end{table}
\begin{table}
\centering 
\begin{tabular}{lrrrr}
...
\bottomrule
\end{tabular}
\label{tabla-comparacion-SSIM}
\caption{SSIM para los distintos metodos de filtrado.}
\end{table}
\begin{figure}[H]
        \centering
        \begin{subfigure}[H]{0.4\textwidth}
...
\end{figure}
\subsection{aplicacion a imagenes reales}

Por último, con el fin de verificar el funcionamiento del filtrado con imágenes reales (es decir que el ruido presente en ellas no se agrego artificialmente). Para esto se utilizaron 3 imágenes, una del Príncipe de Gales del año 1925 y dos resonancias que tienen la particularidad de presentar un tipo de ruido llamado Rician propio del estudio.
...
\begin{figure}[H]
        ...
        \begin{subfigure}[H]{0.4\textwidth}
            \centering
            \includegraphics[width=\textwidth]{Fig/real/2}
            \caption[tom1sinfiltrar.]%
           {{\small Imagen real sin filtrar.}}    
            \label{}
        \end{subfigure}
     %   \hfill
        \begin{subfigure}[H]{0.4\textwidth}  
            \centering 
            \includegraphics[width=\textwidth]{Fig/real/2-f}
            \caption[]%
            {{\small Imagen real filtrada.}}    
            \label{}
        \end{subfigure}
...
A partir de los resultados obtenidos se puede concluir de manera general que el método de filtrado de ruido utilizando la transformada de Wavelet es una herramienta útil. Viendo las figuras \ref{fig-imagenes-prueba}, \ref{fig-imagenes-filtradas} observamos que el resultado de filtrar el ruido sintético es aceptable. Si bien se presenta un difuminado que provoca perdidas de detalles esto es normal en la mayoría de métodos que utilizan umbrales, justamente por la necesidad de eliminar el ruido. Es importante recalcar la elección del nivel de descomposición y su relación con los estadísticos. Si bien en las tablas vemos que el nivel dos es el que brinda mejores resultados numéricamente, a la hora de ver las imágenes el nivel seis muestra menor cantidad de ruido que la imagen del nivel dos. Esta discrepancia se puede deber a que si bien la imagen del nivel 2 presenta ruido visible, la intensidad de los píxeles no varía tanto respecto a la original como si lo hace el nivel 6, lo que genera en la forma de calcular los estadísticos un mejor resultado. Por otro lado, la razón por la cual el nivel seis elimina mayor cantidad de ruido es que el nivel de descomposición aumenta y esto permite filtrar una mayor cantidad de coeficientes de detalles. Como resultado se elimina una cantidad superior de ruido a costa de perder detalles. La wavelet utilizada sym8 es congruente que brinde mejores resultados ya que la forma de este tipo de onda se adapta de manera óptima a los detalles de la imagen por tener una forma mas suave que la wavelet haar y db4. Por último, el algoritmo de umbralización NormalShrink nos brinda un mejor filtrado debido a que utiliza información de la imagen y no solo de la varianza del ruido. \\
Si analizamos en particular las tablas, es interesante el resultado de la tabla \ref{tabla-wavelets}. Para la wavelet haar la casa obtiene mejor PSNR lo que es coherente por la forma de la haar y por los bordes rectos que preesnta la casa. La forma discontinua y ``dura'' de esta wavelet no suaviza los bordes lo que permite que sobrevivan al filtrado. Sin embargo, los detalles de la imagen que no posean estas características se ven suprimidos lo que genera una mayor discrepancia con la imagen original y es congruente con el resultado de la SSIM (tabla  \ref{tabla-wavelets-SSIM})\\
Ahora analizamos los resultados con los métodos de filtrado gaussiano y wiener. Vemos en la figura \ref{fig-comp-metodos} que estos dos métodos no quitan tanto el ruido como el método de la transformada wavelet, pero a raíz de esto presentan un menor difuminado. Si nos guiamos con la tabla \ref{tabla-comparacion-SSIM}, los resultados de la transformada wavelet dan un mejor SSIM comparado con wiener y gaussiano. Concluyendo, la transformada de wavelet utilizada para filtrar ruido actua de manera óptima al compararlo con métodos especializados en el filtrado de ruido.\\
Por último, cuando se implemento el filtrado en imágenes reales que contenían ruido, es decir que no se le agregó ruido de forma sintética, el resultado es aceptable. En la imagen del Príncipe de Gales es destacable que en comparación con las imágenes sintéticas esta no posee el mismo nivel de ruido en todas sus regiones y por lo tanto en las zonas donde es mayor el ruido vemos un mayor difuminado en la imagen resultante. Además, en el caso particular de las resonancias, éstas no presentan ruido gaussiano y aún así el filtro fue lo bastante robusto para adaptarse brindando un buen resultado.
